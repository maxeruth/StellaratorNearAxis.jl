...
function change_order(::IdentityPowerSeries{T}, ::Integer) where {T}
    IdentityPowerSeries(;T)
end
function evaluate(::IdentityPowerSeries{T}, ρ, θ, s) where {T}; ones(T,length(ρ),length(θ),length(s)); end